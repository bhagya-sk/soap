package com.reactiveworks.practice.dao.util;
...
public class DBUtil {
   private static final Logger LOGGER_OBJ=Logger.getLogger(DBUtil.class);
	 ...
	 * @throws ConnectionAccessException 
	 ...
	 */
	public static synchronized Connection getdbconnection() throws ConnectionAccessException {
		...
		if (dataSource == null) {
			...
			try (InputStream input = DBUtil.class.getClassLoader().getResourceAsStream(PROPERTY_FILE)) {
				...
				throw new ConnectionAccessException(exp.getMessage(),exp);
			}
